class Accelerator {
  ...
  updateAccelerators(player, mu) {
    for (let i = 1; i < 8; i++) {
      ...
      if (i == 1 && player.challenge.activeBonuses[10]) {
        mult = player.challenge.rankBonuses[10] ? mult.add(this.acceleratorsBought[i].pow_base(2)) : mult.add(this.acceleratorsBought[i]);
      } else if (i != 1 && player.challenge.rankBonuses[6]) {
        mult = player.challenge.rankBonuses[10] ? mult.add(this.acceleratorsBought[i].pow_base(2)) : mult.add(this.acceleratorsBought[i]);
      }
  ...
  calcSpeed(challenge, chip10) {
    ...
    if (challenge.activeBonuses[6]) {
      if (challenge.rankBonuses[10]) {
  ...
  reset(challenge) {
    ...
    let bonus = 0;
    if (challenge.activeBonuses[1]) bonus += 10;
    if (challenge.rankBonuses[1]) bonus += 256;
