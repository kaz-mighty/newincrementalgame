...
class Player {
   ...
   * @returns {boolean[]}
   */
  static numArray2BoolArray(numArray, length) {
  ...
  static boolArray2numArray(boolArray) {
    let numArray = [];
    for (let i = 0; i < boolArray.length; i++) {
      if (boolArray[i]) numArray.push(i);
    }
    return numArray;
  }
  /** @param {boolean[]} boolArray */
  static countTrue(boolArray) {
    let count = 0;
    for (const flag of boolArray) {
      if (flag) count++;
    }
    return count;
  }
  ...
  toSaveObject() {
    return {
      ...
      challenges: Player.boolArray2numArray(this.challenge.challenges),
      challengecleared: this.challenge.challengeCleared,
      challengebonuses: Player.boolArray2numArray(this.challenge.bonuses),
      ...
      pchallenges: Player.boolArray2numArray(this.challenge.perfectChallenges),
      ...
      rankchallengebonuses: Player.boolArray2numArray(this.challenge.rankBonuses),
  ...
  calcCommonMult() {
    ...
    if (this.challenge.activeBonuses[3]) {
      mult = mult.mul(new Decimal(2));
    }

    if (this.challenge.rankBonuses[3]) {
    ...
    if (this.challenge.rankBonuses[11]) {
      mult = mult.mul(new Decimal(2).pow(new Decimal(this.memorySum).div(x2)));
    }

    mult = mult.mul(1 + Math.sqrt(this.pipedSmallTrophy));

    if (this.challenge.onChallenge && this.challenge.rankBonuses[4]) {
      mult = mult.mul(1 + Player.countTrue(this.challenge.challenges) * 0.25);
    }
  ...
  calcBasicIncrementMult(i) {
    ...
    if (!(this.challenge.isChallengeActive(2))) {
      ...
      if (this.challenge.activeBonuses[11]) {
      ...
      if (i < this.generator.highestGenerator && mm.greaterThanOrEqualTo(1)) {
        ...
        if (this.challenge.activeBonuses[2] && mm.greaterThanOrEqualTo(1)) {
          mult = mult.mul(mm);
        }
      }
    }

    if (i == 0 && this.challenge.activeBonuses[7]) {
      if (this.challenge.rankBonuses[7]) {
  ...
  calcTickSpeed() {
    ...
    tsp -= this.levelShop.levelItems[1] * Player.countTrue(this.challenge.bonuses) * (1 + this.chip.setChip[27] * 0.5);
  ...
  calcGainLevel() {
    ...
    if (this.challenge.activeBonuses[12]) gainLevel = gainLevel.mul(new Decimal(2));
    return gainLevel;
  }
  ...
  resetLevel(force, exit) {
    ...
    let gainLevelReset = rst.mul(1 + this.chip.setChip[20]).mul(new Decimal(exit ? 0 : this.challenge.activeBonuses[8] ? 2 : 1));
    ...
    if (force || confirm('昇段リセットして、段位' + gainLevel + 'を得ますか？')) {
      ...
      if (this.challenge.onChallenge) {
        ...
        if (Player.countTrue(this.challenge.challenges) >= 6) {
      ...
      if (this.challenge.activeBonuses[0]) this.money = new Decimal(10001);
      if (this.challenge.rankBonuses[0]) this.money = this.money.add(new Decimal("1e9"));
    }
  }
  ...
  calcGainRank() {
    ...
    if (this.challenge.rankBonuses[12]) {
  ...
  resetRank(force) {
    ...
    if (force || confirm('昇階リセットして、階位' + gainRank + 'を得ますか？')) {
      ...
      this.rankResetTime = this.rankResetTime.add((this.challenge.rankBonuses[8] ? new Decimal(3) : new Decimal(1)).mul(this.chip.setChip[24] + 1).mul(this.crownResetTime.add(1)));
  ...
  update() {
    this.challenge.activeBonuses = (!this.challenge.onChallenge || this.challenge.bonuses[4]) ? this.challenge.bonuses : new Array(15).fill(false);
    ...
    if (!this.challenge.onChallenge && this.challenge.rankBonuses[14] && this.common.autoRank) {
    ...
    if (this.challenge.rankBonuses[5] && this.common.levelItemAutoBuy) {
      for (let i = 0; i < 5; i++) {
        this.levelShop.buyLevelItems(this, i);
      }
    }

    if ((this.auto.autoDoChallenge || !this.challenge.onChallenge) && this.challenge.activeBonuses[14] && this.common.autoLevel) {
    ...
    if (this.challenge.activeBonuses[5] && this.common.genAutoBuy) {
      for (let i = 7; i >= 0; i--) {
        this.generator.buyGenerator(this, i);
      }
    }

    if (this.challenge.activeBonuses[9] && this.common.accAutoBuy) {
      // bug: 時間加速器8が自動購入されない
  ...
  updateTickSpeed() {
    ...
    if (this.challenge.rankBonuses[9]) {
