function Generatordata() {
  this.calcgncost = function (data) {
    for (let i = 0; i < 8; i++) {
      ...
      if (
        ...
        data.player.challenges.includes(1) &&
        data.player.generatorsBought[i].gt(0)
      ) {
        p = p.mul(2);
      }
  ...
  this.updategenerators = function (data, mu) {
    for (let i = 0; i < 8; i++) {
      if (!data.activechallengebonuses.includes(13)) {
        ...
        if (to === 0) {
          data.player.money = data.player.money.add(
            data.player.generators[i].mul(mult),
          );
        } else {
          data.player.generators[to - 1] = data.player.generators[to - 1].add(
            data.player.generators[i].mul(mult),
          );
        }
      } else {
        if (data.player.onchallenge && data.player.challenges.includes(3)) {
          ...
          for (let to = 0; to <= i; to++) {
            ...
            if (to === 0) {
              ...
              data.player.generators[to - 1] = data.player.generators[
                to - 1
              ].add(data.player.generators[i].mul(mult));
            }
          }
        }
      }
  ...
  this.buyGenerator = function (data, index) {
    ...
    if (
      data.player.money.greaterThanOrEqualTo(data.player.generatorsCost[index])
    ) {
      data.player.money = data.player.money.sub(
        data.player.generatorsCost[index],
      );
      data.player.generators[index] = data.player.generators[index].add(1);
      data.player.generatorsBought[index] =
        data.player.generatorsBought[index].add(1);
  ...
  this.calccommonmult = function (data) {
    ...
    if (!(data.player.onchallenge && data.player.challenges.includes(7))) {
      let cap = new Decimal(100).mul(
        data.player.levelitems[2] * (1 + data.player.setchip[28] * 0.3) + 1,
      );
    ...
    if (
      data.player.onchallenge &&
      data.player.rankchallengebonuses.includes(4)
    ) {
    ...
    if (!(data.player.onpchallenge && data.player.pchallenges.includes(8))) {
      if (data.player.darkmoney.greaterThanOrEqualTo(1)) {
        mult = mult.mul(
          new Decimal(data.player.darkmoney.add(10).log10()).pow(
            1 + data.player.setchip[40] * 0.1,
          ),
        );
      }
    }
    ...
    if (data.player.activatedcampaigns.includes("newyear2025")) {
      if (
        data.player.onchallenge &&
        data.player.challenges.includes(3) &&
        data.player.challenges.includes(4)
      ) {
        camp = camp + 10;
      }
    }
  ...
  this.calcincrementmult = function (data, i, to) {
    ...
    let lv = new Decimal(
      data.player.level
        .pow(1 + 0.5 * data.player.setchip[19])
        .add(2)
        .log2(),
    );
    ...
    rk +=
      new Decimal(data.player.rank.add(2).log2()).log2() *
      data.player.setchip[23];
    mult = mult.mul(
      new Decimal(lv.pow((i - to) * (1 + Math.max(rk, 0) * 0.05))),
    );

    if (
      data.player.onpchallenge &&
      data.player.pchallenges.includes(3) &&
      mult.gt("1e-100")
    ) {
  ...
  this.calcbasicincrementmult = function (data, i) {
    ...
    if (!(data.player.onchallenge && data.player.challenges.includes(2))) {
      ...
      if (i < data.highest && mm.greaterThanOrEqualTo(1)) {
        ...
        if (
          data.activechallengebonuses.includes(2) &&
    ...
    if (i == 0 && data.activechallengebonuses.includes(7)) {
      if (data.player.rankchallengebonuses.includes(7)) {
        mult = mult.mul(
          data.strongsoftcap(data.player.maxlevelgained, new Decimal(100000)),
        );
      } else {
    ...
    if (!(data.player.onpchallenge && data.player.pchallenges.includes(8))) {
      if (data.player.darkgenerators[i].greaterThanOrEqualTo(1)) {
        mult = mult.mul(
          new Decimal(i + 2 + data.player.darkgenerators[i].log10()).pow(
            1 + data.player.setchip[i + 32] * 0.25,
          ),
        );
      }
    }
