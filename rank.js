function Rankdata() {
  ...
  this.calcgainrank = function (data) {
    ...
    let dv = 36 - 0.25 * data.player.rememberSum - 1.2 * data.player.levelShop.levelItems[4] * (1 + 0.2 * data.player.setChip[29])
    dv = Math.max(dv, 6)
    ...
    gainrank = gainrank.mul(1 + data.player.eachPipedSmallTrophy[4] * 0.2)
    return gainrank
