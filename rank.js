  ...
  this.resetRank = function (data, force) {
    ...
    if(force || confirm('昇階リセットして、階位' + gainrank + 'を得ますか？')){
      ...
      if(data.player.onchallenge) {
        ...
        if(data.player.challengecleared.length>=128 && !data.player.rankchallengecleared.includes(data.calcchallengeid())){
      ...
      data.player.rankresettime = data.player.rankresettime.add((data.player.rankchallengebonuses.includes(8)?new Decimal(3):new Decimal(1)).mul(data.player.setchip[24]+1).mul(data.player.crownresettime.add(1)))
